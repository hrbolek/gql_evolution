...
class DisciplineSetGQLModel(BaseGQLModel):
    ...
    @strawberry.field(description="Returns a disciplines for the discipline set")
    async def disciplines(self, info: strawberry.types.Info, id: uuid.UUID) -> typing.List[DisciplineGQLModel]:
        from .DisciplineGQLModel import DisciplineGQLModel
        result = await DisciplineGQLModel.load_with_loader(info=info, id=id)
        return result
    
    @strawberry.field(description="Returns a result templates of the discipline set")
